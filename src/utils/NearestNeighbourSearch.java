...
public abstract class NearestNeighbourSearch implements Serializable {
	...
	protected class MyHeap {
		...
		public void putBySubstitute(int i, double d) throws Exception {
			...
			put(i, d);
			// System.out.println("previous: "+head.distance+" current:
			// "+m_heap[1].distance);
			if (head.distance == m_heap[1].distance) { // Utils.eq(head.distance, m_heap[1].distance)) {
				...
				throw new Exception("The substituted element is smaller than the "
						+ "head element. put() should have been called " + "in place of putBySubstitute()");
			}
		}
	 ...
	 * We use a list so that we can take care of cases where multiple neighbours are
	 * the same distance away. i.e. the minimum length of the list is k.
	 *
	 * @author Ashraf M. Kibriya (amk14[at-the-rate]cs[dot]waikato[dot]ac[dot]nz)
	 * @version $Revision: 8034 $
	...
	protected class NeighborList {
		...
		/**
		 * Inserts an instance neighbor into the list, maintaining the list sorted by
		 * distance.
		 *
	 ...
	 * nearestNeighbour needs to be called first for this to work.
	 *
	...
	/**
	 * Adds information from the given instance without modifying the datastructure
	 * a lot.
	 * 
